...
def main(is_mp):
    env_config = srl.envs.Config("ALE/Freeway-v5")
    ...
    if not is_mp:
        # sequence training
        config.set_train_config(timeout=30, callbacks=[PrintProgress()])
        ...
        mp_config.set_train_config(
            timeout=60 * 10, callbacks=[TrainFileLogger(enable_log=True, enable_checkpoint=False)]
        )
    ...
    # save animation
    render.create_anime().save("tmp/Freeway.gif")
