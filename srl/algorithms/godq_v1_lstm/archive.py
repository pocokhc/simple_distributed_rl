...
class ArchiveCell:
    ...
    def set(self, start_state, steps, step, reward, backup, dist):
...
class Archive:
    ...
    def on_reset(self, worker: WorkerRun, rl_worker: "Worker"):
        ...
        if random.random() < self.config.archive_rate:
            ...
            if cell is not None:
                ...
                # hc
                hc = self.net.encoder.get_initial_state(1)
                for state, act in cell.steps:
                    oe, hc = self.net.pred_oe(state[np.newaxis, ...], act, hc)
                rl_worker.hc = hc
                rl_worker.oe = oe
                rl_worker.q = self.net.pred_q(oe)[0]
    ...
    def find_nearest_archive(self, state: np.ndarray):
        ...
        for v in self.archive_cells.values():
            for cells in v.values():
                for c in cells:
                    dist = np.linalg.norm(c.steps[-1][0] - state)
