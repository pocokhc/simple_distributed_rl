...
def test_1dim():
    ...
    np.testing.assert_array_equal(de, np.array([1.1], np.float32))
    ...
    np.testing.assert_array_equal(de, np.array([1.1], np.float32))
    en = space.encode_to_np(np.array([1.2]), np.float32)
...
def test_1dim_discrete():
    ...
    # --- eq
    assert space == BoxSpace((3,), 0, [2, 5, 3], np.int64)
    assert space != BoxSpace((3,), 0, [3, 5, 3], np.int64)
...
def test_2dim_discrete():
    ...
    de = space.decode_from_list_float([1.1, 2.2, 3.0, -1.0, 1.5, 1.6])
    assert de.shape == (3, 2)
    np.testing.assert_array_equal(de, np.array([[1, 2], [3, -1], [1, 1]], np.int64))
    en = space.encode_to_list_float(np.array([[1, 2], [3, -1], [1, 1]], np.int64))
    assert isinstance(en, list)
    for n in en:
        assert isinstance(n, float)
    np.testing.assert_array_equal(en, [1.0, 2.0, 3.0, -1.0, 1.0, 1.0])
    ...
    assert de.shape == (3, 2)
    np.testing.assert_array_equal(de, np.array([[1, 2], [3, -1], [1, 1]], np.int64))
    en = space.encode_to_np(np.array([[1, 2], [3, -1], [1, 1]]), np.int64)
    assert en.shape == (3, 2)
    np.testing.assert_array_equal(en, np.array([[1.0, 2.0], [3.0, -1.0], [1.0, 1.0]], dtype=np.float32))
    ...
    # --- eq
    assert space == BoxSpace((3, 2), -1, 3, np.int64)
    assert space != BoxSpace((3, 2), -1, 2, np.int64)
    assert space != BoxSpace((3, 3), -1, 3, np.int64)
